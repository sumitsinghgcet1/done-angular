 ...
 */
 angular.module('storeClientApp').controller('OrdersCtrl', function ($scope, dataFactory, ruleService, orderRules){
    $scope.isGrid = 1;
    $scope.entityList = [];
    ...
    $scope.saveOrder = function(edit){
        ...
        var errorMessage = ruleService.validatePizzaOrderMandatoryElements(data);
        if (errorMessage){
          $scope.error = 1;
